...
function get_post_parents(string $post_type, string $search = '', ?string $value = null) {
	if($value !== null && $value !== '') {
		...
		return $post ? [[
			...
			'label' => ($post->post_title !== '' ? $post->post_title : __('(no title)', 'arcanum-admin-query-filters')).' (#'.$post->ID.')'
		]] : [];
	}
	...
	return array_map(fn($row) => [
		'value' => (string) $row->ID,
		'label' => ($row->post_title !== '' ? $row->post_title : __('(no title)', 'arcanum-admin-query-filters')).' (#'.$row->ID.')'
	], $rows);
}
